...
select *
from patients p
where p.doctor_id in (select d.id
                      from doctors d
                      where d.department_id in (select d.id from departments d where d.department_name = 'Surgical'));

--2.Получить список всех отделений, где работает врач с именем 'Miller’
select *
from departments d
where d.id in (select department_id from doctors d where d.first_name = 'Miller');

--3.Найти всех пациентов, у которых врач имеет стаж более 10 лет
select *
from patients p
where p.doctor_id in (select id from doctors d where experience > 10);
...
select *
from patients p
where p.doctor_id in (select p.doctor_id from doctors);
...
select *
from doctors d
         inner join patients p on d.id = p.doctor_id
where doctor_id is null;
...
select *
from patients p
where p.doctor_id in (select doctor_id from doctors d where d.first_name = 'Conrado' and d.last_name = 'Placide');

--9.Получить список всех врачей, работающих в отделении 'Intensive' и обслуживающих больше 3 пациентов
select *, department_name, p.first_name, count(p.id)
from doctors d
         inner join departments d2 on d2.id = d.department_id
         inner join patients p on d.id = p.doctor_id
where d.first_name = 'Intensive'
group by department_name, p.first_name
having count(p.id) > 3;
...
select *
from departments d
where d.id in (select d.department_id
               from doctors d
...
select *
from patients p
where p.doctor_id in (select doctor_id from doctors d where d.specialization = 'DERMOTOLOGIST');

--12.Получить количество врачей для каждой специализации
select d.specialization, count(d.id) as count_doctor
from doctors d
group by specialization;

--12.Найти всех пациентов, обслуживаемых врачами с наименее распространенной специализацией
select *
from patients p
where p.doctor_id in (select doctor_id from doctors d group by specialization order by count(*) limit 1);
--13.Найти всех пациентов, обслуживаемых врачами с специализацией ‘CARDIOLOGIST’
select *
from patients p
         inner join doctors d on d.id = p.doctor_id
where d.specialization = "CARDIOLOGIST";

--14.Получить список всех врачей, работающих в отделении 'Neurology' и обслуживающих больше 3 пациентов
select d.first_name
from doctors d
         join departments d2 on d2.id = d.department_id
         join patients p on d.id = p.doctor_id
where d.first_name = 'Neurology'
group by d.id
having count(p.id) > 3;
